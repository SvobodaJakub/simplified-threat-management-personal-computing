...
class AttackVector:
    ...
    #   * impossible
    #   * hard - The attacker has to perform a non-trivial procedure and/or spend considerable time tinkering with the device and/or do an obvious suspicious action in a monitored space - e.g. take off the case & install a SIM card & turn off airplane mode & activate SIM card & do a further action
    #   * easy - The attacker has to perform a simple quick action - e.g. plug a usb, turn off airplane mode, steal the device
    #   * trivial - e.g. no lockscreen, no encryption.
    # Nearby attack - the attacker has to be in the vicinity of the device
    #   * impossible
    #   * hard - The attacker has to perform a non-trivial procedure and/or spend considerable time with the procedure and/or do an obvious suspicious action in a monitored space - e.g. wait nearby until the SIM card is turned on and logged to the network and then get root on the device within 2 minutes
    #   * easy - The attacker has to perform a simple quick action - e.g. plug a usb, turn off airplane mode, steal the device; e.g. (future-wise assumption) plant a scripted fake base station near workplace that uses a slow method to get root on a permanently running device with GSM enabled
    ...
    #   * impossible
    #   * hard - The attacker has to perform a non-trivial procedure and/or spend considerable time with the procedure - e.g. get root through attack on TLS chained with code execution
    #   * easy - The attacker has to perform a simple action - e.g. rent an exploit kit, insert it into an ad, display it on the device
    #   * trivial - e.g. open telnet login without password, insecure services listening unfirewalled
    def __init__(self, max_sec_phys, max_sec_near, max_sec_remote):
        # maximum achievable security when the attack vector is considered (the applicable device can't have higher security than that)
...
#   * e.g. getting data off a locked encrypted device that is not vulnerable through connected network or USB
...
#   * e.g. (future-wise assumption) rooting into a phone through GSM that is running with a valid SIM card all the time
#   * e.g. code execution on device through an ad-laden app with excessive permissions, or through an insecurely made app

# Use your own judgement here.
# Even a remote attack can somehow facilitate a physical attack, in theoretical scenarios stretched enough.
attack_net_hard = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)
attack_net_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_EASY)

# A physical attack can theoretically facilitate a remote attack, e.g. by disabling airplane mode.
attack_phys_hard = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)
attack_phys_easy = AttackVector(max_sec_phys=DIFF_EASY, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_HARD)

# 2 minutes of GSM use for banking 2FA SMS.
attack_gsm_hard = AttackVector(max_sec_phys=DIFF_IMPOSSIBLE, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)

# Normal mobile phone use.
attack_gsm_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_EASY)

# Hardened simple apps like FreeOTP, banking app, keepass.
attack_app_hard = AttackVector(max_sec_phys=DIFF_IMPOSSIBLE, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)

# Normal apps with carefully selected permissions.
attack_app_medium = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_EASY)

# Junk apps, apps with ads, apps with too many permissions, suspicious apps.
attack_app_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_EASY)

# Attacker sees encrypted traffic or attacker can execute code on a logged-in android device (but not on a logged-in desktop).
attack_google_hard = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)

# Attacker can access the google account from a logged-in desktop, can sniff & spoof the password and 2FA.
attack_google_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_EASY)

# Attacker gets root inside the wifi radio chip and manipulates traffic.
attack_wifi_hard = AttackVector(max_sec_phys=DIFF_IMPOSSIBLE, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)

# Attacker takes advantage of insecure app that listens, exploits it to unlock the device, or exploits it to get root.
# OR attacker can manipulate plaintext traffic.
attack_wifi_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_HARD)

# The bluetooth assumptions are somewhat wrong and specific to a certain usecase, do use your own judgement.

# Attacker exploits the device with enabled bluetooth while the device is not trying to connect with anything and is not in discoverable mode and wifi is disabled (or doesn't have neighboring MACs).
attack_bluetooth_hard = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_HARD)

# Attacker exploits the device with enabled bluetooth while the device is communicating and bluetooth is dicoverable / wifi is enabled (neighboring MACs)
# Also assuming that the attacker installs code that allows remote control.
attack_bluetooth_easy = AttackVector(max_sec_phys=DIFF_HARD, max_sec_near=DIFF_EASY, max_sec_remote=DIFF_EASY)
...
class Usecase:
    ...
    # A usecase can also collide with other usecases - it cannot be performed on the device if a specific colliding usecase is also performed on the device
    def __init__(self, name, req_min_sec_phys=DIFF_EASY, req_min_sec_near=DIFF_EASY, req_min_sec_remote=DIFF_EASY, introduces_attack_vectors=None, colliding_usecases=None):
        self.name = name
        self.req_min_sec_phys = req_min_sec_phys
        self.req_min_sec_near = req_min_sec_near
        self.req_min_sec_remote = req_min_sec_remote
        self.introduces_attack_vectors = set(introduces_attack_vectors) if introduces_attack_vectors else set()
...
usecase_freeotp_personal = Usecase(name="usecase_freeotp_personal", 
                                    req_min_sec_phys=DIFF_EASY, req_min_sec_near=DIFF_EASY, req_min_sec_remote=DIFF_EASY, 
...
# Even if we ignore the fact that everyday GSM use lowers the device's security (which this script should calculate),
# having a separate unknown number for banking is good security through obscurity. Ignoring 2-SIM phones here.
# Banking 2FA SMS relies on an insecure channel - the GSM network (and related networks) - so the REAL minimum required security
# is sadly low.
...
usecase_banking_gsm_sim = Usecase(name="usecase_banking_gsm_sim", 
                                    req_min_sec_phys=DIFF_EASY, req_min_sec_near=DIFF_EASY, req_min_sec_remote=DIFF_EASY, 
                                    introduces_attack_vectors=[attack_gsm_hard], colliding_usecases=[usecase_personal_gsm_sim])
...
usecase_banking_app = Usecase(name="usecase_banking_app", 
                                    ...
                                    introduces_attack_vectors=[attack_net_hard, attack_app_hard, attack_wifi_hard],
                                    colliding_usecases=[usecase_banking_gsm_sim, usecase_personal_gsm_sim])

# Personal google account is for everyday use on those kinds of devices that can be remotely attacked e.g. through exploits in ads.
# Assuming it is non-trivial to misuse a logged in account on an android device to surreptitiously install apps on another device
...
# Personal google account is for everyday use on those kinds of devices that can be remotely attacked e.g. through exploits in ads.
# Assuming it is trivial to misuse a logged in account on a desktop computer to surreptitiously install apps on another device
# Banking SIM also used for recovery of the google account, so it can't be on the same device.
# Personal FreeOTP has 2FA for the personal google account, so it can't be on the same device.
...
# This is the keepass that largely contains web logins that are also saved in a web browser, so I have to be realistic, that it
# is for use on a device exploitable e.g. through malicious ads.
# Must not be used on the same device where the accompanying 2FA app is used. (so that the two factors are separate)
...
usecase_personal_keepass = Usecase(name="usecase_personal_keepass", 
                                    req_min_sec_phys=DIFF_HARD, req_min_sec_near=DIFF_EASY, req_min_sec_remote=DIFF_EASY, 
                                    introduces_attack_vectors=[attack_app_hard], 
                                    colliding_usecases=[usecase_personal_gsm_sim, usecase_freeotp_personal])

usecase_junk_apps = Usecase(name="usecase_junk_apps", 
                                    req_min_sec_phys=DIFF_TRIVIAL, req_min_sec_near=DIFF_TRIVIAL, req_min_sec_remote=DIFF_TRIVIAL, 
                                    introduces_attack_vectors=[attack_app_easy, attack_net_easy, attack_wifi_easy], 
                                    colliding_usecases=[])
...
usecase_personal_computing = Usecase(name="usecase_personal_computing", 
                                    req_min_sec_phys=DIFF_EASY, req_min_sec_near=DIFF_EASY, req_min_sec_remote=DIFF_EASY, 
                                    introduces_attack_vectors=[attack_app_hard, attack_net_easy, attack_wifi_hard], 
                                    colliding_usecases=[usecase_personal_gsm_sim, usecase_junk_apps])

usecase_secure_computing = Usecase(name="usecase_secure_computing", 
                                    req_min_sec_phys=DIFF_HARD, req_min_sec_near=DIFF_HARD, req_min_sec_remote=DIFF_HARD, 
                                    introduces_attack_vectors=[attack_app_hard, attack_net_hard, attack_wifi_hard], 
                                    colliding_usecases=[usecase_personal_gsm_sim, usecase_banking_gsm_sim])
...
usecase_too_old_android = Usecase(name="usecase_too_old_android", 
                                    colliding_usecases=[usecase_personal_google_account_computer, usecase_banking_app, usecase_junk_apps,
                                                        usecase_personal_keepass, usecase_secure_keepass, usecase_secure_google_account,
                                                        usecase_personal_google_account_android, usecase_personal_gsm_sim,
                                                        usecase_personal_computing, usecase_secure_computing])

# All operating systems and their different browsers have no ad blocker whatsoever, so that's at least one reason
# why the minimum required remote security is only DIFF_EASY. Also, TLS is used, so the minimum required nearby
...
class Device:
    ...
    def __init__(self, name, pinned_usecases=None, additional_usecases=None, max_sec_phys=DIFF_IMPOSSIBLE, max_sec_near=DIFF_IMPOSSIBLE, max_sec_remote=DIFF_IMPOSSIBLE):
        self.name = name
    ...
    def calculate_usecase_sec_reqs(self):
        ok = True
        sp, sn, sr = self.calculate_usecase_max_sec()
        # print(repr(self.calculate_usecase_max_sec()))
        usecases = self.pinned_usecases | self.additional_usecases
        for u in usecases:
            # print (u.name)
...
primary_pocket_computer = Device(name="primary_pocket_computer", 
                                    pinned_usecases=[usecase_android_device, usecase_personal_computing, 
                                                        usecase_personal_google_account_android, usecase_personal_keepass,
...
primary_laptop_computer = Device(name="primary_laptop_computer", 
                                    pinned_usecases=[usecase_computer, usecase_nongsm_device, usecase_nonotp_device, 
                                                        usecase_personal_computing, usecase_personal_keepass, usecase_personal_google_account_computer, ],
                                    max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)
...
phone_banking_sim = Device(name="phone_banking_sim", 
                                    ...
                                    # it's an old phone, it has to be powered off when not in use to mitigate most of the attack surface
...
android_secure = Device(name="android_secure", 
                                    ...
                                    max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)
android_b = Device(name="android_b", 
                                    pinned_usecases=[usecase_android_device],
                                    max_sec_phys=DIFF_HARD, max_sec_near=DIFF_HARD, max_sec_remote=DIFF_HARD)

#devices = [primary_pocket_computer, primary_laptop_computer, gsm_phone, computer_sec, phone_banking_sim, android_secure, android_b]
...
# NOTE - The initial check can be used for experimenting and to incrementally find out which pinned usecases work together and which don't.
print("Initial check - the set of devices and their pinned usecases can work: " + repr(device_assignment_ok(devices)))
